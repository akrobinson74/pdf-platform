...
public class PdfMutationResolver implements GraphQLMutationResolver {
  ...
  private static void storeAndSaveMetadata(
    ...
    try {
      ...
      final var copyPath = Paths.get(
          String.join(File.separator, newDirPath, StringUtils.cleanPath(newFilename)));

      log.info("Copying inputStream named {} to: {} ({} bytes)", fileName, copyPath,
          filePart.getSize());
  ...
  private PdfMetadata parseFilename(String fileName) {
    ...
    return new PdfMetadata(nameParts[0], nameParts[1], fileName, nameParts[2],
        ReportType.valueOf(nameParts[3]));
  }
}
